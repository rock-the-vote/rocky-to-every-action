...
def normalize_row(row):
    ...
    return {
        ...
        "home_address":            (row.get("home_address") or row.get("address")
                                   or " ".join(filter(None, [row.get("street_number"), row.get("street_name")])) or None),
        ...
        "optin_to_partner_sms":    row.get("optin_to_partner_smsrobocall") or row.get("partner_opt_in_sms"),
        "volunteer_for_partner":   row.get("volunteer_for_partner"),
...
def build_person(row):
    ...
    # Only set isSubscribed when Rocky has an explicit opt-in — omitting it leaves
    # any existing EA subscription status untouched rather than overwriting with false.
    email_address = row.get("email_address")
    email = None
    if email_address:
        email_obj = {"email": email_address}
        if parse_bool(row.get("optin_to_partner_email")):
            email_obj["isSubscribed"] = True
        email = [email_obj]
    ...
    person = {
        ...
        "email":         email,
        "phone":         row.get("phone"),
        "phone_type":    PHONE_TYPE_MAP.get(rocky_phone_type),
        "date_of_birth": dob,
    ...
    # Drop None and empty-string values so blank Rocky fields don't overwrite existing EA data
